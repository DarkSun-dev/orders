...
exports.login = catchAsync(async (req, res, next) => {
  ...
  const user = await User.findOne({ telefone }).select('+password');
...
exports.updatePassword = catchAsync(async (req, res, next) => {
  ...
  if (!(await user.correctPassword(req.body.passwordCurrent, user.password))) {
    res.json({ message: 'O seu actual password esta incorrecto!', code: 401 })
  } else {
    user.password = req.body.password;
    user.passwordConfirm = req.body.passwordConfirm;
    await user.save();
...
exports.deleteReg = async (req, res, next) => {
  const pass = await Reset.deleteOne({ telefone: req.params.telefone })
}